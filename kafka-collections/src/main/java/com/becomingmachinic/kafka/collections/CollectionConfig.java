...
public class CollectionConfig {
	...
	/**
	 * When strong local consistency is required it is recommended to use {@value CollectionConfig#COLLECTION_WRITE_MODE} {@value CollectionConfig#COLLECTION_WRITE_MODE_BEHIND}. When strong topic consistency is more important then local consistency it is
	 * recommended to use {@value CollectionConfig#COLLECTION_WRITE_MODE} {@value CollectionConfig#COLLECTION_WRITE_MODE_AHEAD}.
	 */
	...
	/**
	 * Sends data to Kafka topic. The collection consumer will receive the update from the topic and update the local collection. Values will be sent to the Kafka topic even if they are already present in the local instance.
	 */
	...
	public CollectionConfig(Map<String, Object> configurationMap) throws KafkaCollectionConfigurationException {
		...
		if (configurationMap.get(COLLECTION_NAME) == null || !(configurationMap.get(COLLECTION_NAME) instanceof String) || ((String) configurationMap.get(COLLECTION_NAME)).isEmpty()) {
			throw new KafkaCollectionConfigurationException("Parameter %s is required",
					COLLECTION_NAME);
		}
		if (!((String) configurationMap.get(COLLECTION_NAME)).replaceAll("[^0-9A-Za-z_-]+", "").equals(configurationMap.get(COLLECTION_NAME))) {
		...
		if (configurationMap.get(COLLECTION_TOPIC) != null && !((String) configurationMap.get(COLLECTION_TOPIC)).replaceAll("[^0-9A-Za-z_-]", "").equals(configurationMap.get(COLLECTION_TOPIC))) {
